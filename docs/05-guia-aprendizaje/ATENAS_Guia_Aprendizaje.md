...
*Fase de diseño de API REST cerrada.*

---

# FASE 4 — Implementación Backend: Spring Boot

> Stack confirmado: Java 21 + Spring Boot 3.x + Spring Data JPA/Hibernate +
> Spring Security/JWT + Gradle. PostgreSQL 15 en Docker (ya configurado).
...
**Precisión conceptual importante:** Spring Boot NO hace que el flujo entre capas sea automático. Ese flujo (Controller llama a Service, Service llama a Repository) lo seguís escribiendo vos. Lo que Spring hace es **crear los objetos y conectarlos** sin que vos hagas `new`. La arquitectura en capas es una convención que Spring facilita, no algo que el framework ejecute por sí solo.
...
*Guía actualizada al inicio de la etapa de Implementación Backend.*
*Próximo hito: creación del proyecto con Spring Initializr y conexión a PostgreSQL.*
